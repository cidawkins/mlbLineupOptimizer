...
def lineupBuilder(players, salaryCap, lineups):
    ...
    solver.Add(1 <= (pgCount + pgsgCount) <= 4)
    solver.Add(1 <= (sgCount + sgsfCount + pgsgCount) <= 7)
    solver.Add(1 <= (sfCount + sgsfCount + sfpfCount) <= 6)
    solver.Add(1 <= (pfCount + pfcCount + sfpfCount) <= 5)
    solver.Add(1 <= (cCount + pfcCount) <= 4)
    solver.Add(pgCount <= 3)
    solver.Add(sgCount <= 3)
    solver.Add(sfCount <= 3)
    solver.Add(pfCount <= 3)
    solver.Add(cCount <= 2)

    # Constraint for picking at least 3 guards and forwards
    solver.Add(pgCount + pgsgCount + sgCount + sgsfCount >= 3)
    solver.Add(sfCount + pfCount + pfcCount + sfpfCount + sgsfCount >= 3)
    """
    ...
    for i in range(0, len(lineups)):
        solver.Add(lCrossC[i] + lCrossSG[i] + lCrossSGSF[i] + lCrossSF[i] + lCrossSFPF[i] + lCrossPF[i] +
                   lCrossPFC[i] + lCrossPG[i] + lCrossPGSG[i] <= 5)

    # Max 4 players per team
    for i in range(0, 29):
        solver.Add(teamsPG[i] + teamsPGSG[i] + teamsC[i] + teamsPF[i] + teamsPFC[i] + teamsSG[i] + teamsSGSF[i] +
                   teamsSF[i] + teamsSFPF[i] <= 4)

    solver.Maximize(valueC + valuePG + valuePGSG + valuePF + valuePFC + valueSG + valueSGSF + valueSF + valueSFPF)
    solver.Solve()
    ...
    rPGSG = range(0, len(tempLineup[2]))
    rPF = range(0, len(tempLineup[3]))
    rPFC = range(0, len(tempLineup[4]))
    rSG = range(0, len(tempLineup[5]))
    rSGSF = range(0, len(tempLineup[6]))
    rSF = range(0, len(tempLineup[7]))
    rSFPF = range(0, len(tempLineup[8]))
